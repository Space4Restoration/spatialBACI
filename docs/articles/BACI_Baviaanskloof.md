...
included in the `spatialBACI` package, to demonstrate a pixel-based
...
Hartland Bawarea Conservancy, South-Africa. Attributes the vector
dataset indicate in which year the revegetation was undertaken by the
...
implemented as a restoration activity. As an example, we here select the
sites revegetated in 2012 that were not subject to lifestock grazing
exclusion.
...
![](BACI_Baviaanskloof_files/figure-html/unnamed-chunk-2-1.png)

## Defining impact and control units

The `spatialBACI` package requires the user to define the spatial unit
...
`spatialBACI` package provides several options for this. The first
...
knowledge and data availability in the
[`create_control_candidates()`](https://space4restoration.github.io/spatialBACI/reference/create_control_candidates.md)
...
`exclude_impact_buffer` to exclude pixels in a buffer of 300 m around
...
of the impact sites will be used. Setting the `round_coords` argument to
-2 sets the origin of the output SpatRaster to the closest 100 m, and
setting the `sample_control` argument to 0.5 results in a random
sampling of 50% of the candidate control pixels, which can be useful to
reduce computational load for large study areas.
...
![](BACI_Baviaanskloof_files/figure-html/unnamed-chunk-3-1.png)
...
general, the `spatialBACI` package will automatically check the crs of
...
a restoration intervention). This is in the `spatialBACI` package done
...
al., 2019](https://onlinelibrary.wiley.com/doi/abs/10.1111/cobi.13448)).
...
Elevation Model (DEM) based on SRTM data using the
[`dem()`](https://space4restoration.github.io/spatialBACI/reference/dem.md)
...
![](BACI_Baviaanskloof_files/figure-html/unnamed-chunk-4-1.png)
...
[`lulc()`](https://space4restoration.github.io/spatialBACI/reference/lulc.md)
function, the 9-class land use land cover layer from Impact Observatory
is used. This product is first generated for the year 2017, so we assume
no change in land cover change between the intervention year (2012) and
2017. Given that terrain revegetated with spekboom will still be
classified as “rangeland” this is not an issue in this example, but it
may be a problem for other situations.
...
![](BACI_Baviaanskloof_files/figure-html/unnamed-chunk-5-1.png)

Distance to roads or settlements can explain the anthropogenic pressure
on an area. The `spatialBACI` package therefore includes functions to
calculate the distance to the nearest road
([`osm_distance_roads()`](https://space4restoration.github.io/spatialBACI/reference/osm_distance_roads.md))
or nearest settlement
([`osm_distance_places()`](https://space4restoration.github.io/spatialBACI/reference/osm_distance_places.md))
based on OpenStreetMap data. For this example, we include the distance
to the nearest road of OSM category “track” or higher as an
environmental covariate in the matching analysis.
...
![](BACI_Baviaanskloof_files/figure-html/unnamed-chunk-6-1.png)

The different matching covariates can be easily collated into a format
that can serve as input to the control-impact matching with the
[`collate_matching_layers()`](https://space4restoration.github.io/spatialBACI/reference/collate_matching_layers.md)
function. This function handles the necessary spatial processing of the
different spatial input layers, which may have different coordinate
...
The function
[`test_multicollinearity()`](https://space4restoration.github.io/spatialBACI/reference/test_multicollinearity.md)
plots a correlation matrix of the matching covariates, reports their
Variance Inflation Factor (VIF), and allows the user to iteratively
identify and remove highly multicollinear covariates. In our example,
VIF values for all covariates are low (\<5), so we choose to keep all of
them in the matching.

``` r
matching_input = test_multicollinearity(matching_input)
```

We then perform the control-impact matching with the
[`matchCI()`](https://space4restoration.github.io/spatialBACI/reference/matchCI.md)
function using propensity score matching (default method), and select 10
control pixels for each impact pixel (argument `ratio`) with replacement
(argument `replace`), meaning that a control pixel can be paired with
several impact pixels.
...
performs some standard assessments of the matching output. Under the
default parameters, the function generates plots of the covariate
distribution, and a Love plot of Standardized Mean Differences for
control and impact sites before and after matching. From the output of
the matching analysis, we can see that the absolute Standardized Mean
Difference after matching is below the threshold of 0.1 for the
propensity score distance, only barely above this threshold for a few
individual covariates. Based on these outputs we can continue our
analysis with the current matching output, as we do here, or reject the
matching and attempt matching with different matching parameters or
covariates.
...
![](BACI_Baviaanskloof_files/figure-html/unnamed-chunk-10-1.png)

    #> Press <Enter> to continue.

![](BACI_Baviaanskloof_files/figure-html/unnamed-chunk-10-2.png)

    #> Press <Enter> to continue.

![](BACI_Baviaanskloof_files/figure-html/unnamed-chunk-10-3.png)

    #> Press <Enter> to continue.

![](BACI_Baviaanskloof_files/figure-html/unnamed-chunk-10-4.png)

    #> Press <Enter> to continue.

![](BACI_Baviaanskloof_files/figure-html/unnamed-chunk-10-5.png)

    #> Press <Enter> to continue.

![](BACI_Baviaanskloof_files/figure-html/unnamed-chunk-10-6.png)

    #> Press <Enter> to continue.

![](BACI_Baviaanskloof_files/figure-html/unnamed-chunk-10-7.png)
...
periods before and the 5-year after the intervention year, we can first
...
have a cloud cover below 60%. The function
[`eo_VI_yearly.stac()`](https://space4restoration.github.io/spatialBACI/reference/eo_VI_yearly.stac.md)
above generates proxy data cubes, which don’t contain any data but
describe the dimension of the object. Data retrieval and calculations
are only performed when the data values contained in these objects are
needed. Working with data cubes has the advantage that unnecessary
calculations are avoided for bands, time steps and/or coordinates that
are not used in later steps. This can be extremely useful when dealing
with large areas from which only limited data has to be extracted, e.g.,
sparse monitoring location distributed over a continental-scale area. In
such a case you do not want to process data wall-to-wall over the
continental scale, but only over these sparse locations. In our
Baviaanskloof example, however, we only cover a geographically small
area with a relatively dense cover of impact and candidate control
units. In such case it can be useful to transform the data cubes to
`SpatRaster` objects (from `terra` package) with the
[`as.SpatRaster()`](https://space4restoration.github.io/spatialBACI/reference/as.SpatRaster.md)
function. Doing this downloads and processes all data and reads the
resulting composite images into memory (or writes them to temporal
files).
...
![](BACI_Baviaanskloof_files/figure-html/unnamed-chunk-13-1.png)

``` r
plot(vi_after, main=paste0("NDVI - ",seq(year+1, year+10, 1)))
```

![](BACI_Baviaanskloof_files/figure-html/unnamed-chunk-13-2.png)
...
`calc_ts_metric()` function. This basic time series analysis is included
in the `spatialBACI` package, but user can easily implement their own
analyses, as long as these return raster layers of the evaluation
variable of interest before and after intervention.
...
pairs (output of
[`matchCI()`](https://space4restoration.github.io/spatialBACI/reference/matchCI.md))
and the raster datasets of the `before` and `after` period to the
[`BACI_contrast()`](https://space4restoration.github.io/spatialBACI/reference/BACI_contrast.md)
function. The BACI analysis will be performed for all layers in the
`before` and `after` layers, which in our example are both the average
and trend over the time series.
...
![](BACI_Baviaanskloof_files/figure-html/unnamed-chunk-17-1.png)
